...
def remove_isolated_noise(swc_tree, resize_radio=1.0, isolated_branch_length_max=50):
	branch_list = []
	branch_list_temp = []

	branch_node_list = []
	branch_node_list_temp = []

	swc_tree_list = swc_tree.get_node_list()

	test_tree_rtree = edge_match_utils.get_edge_rtree(swc_tree)
	test_tree_idedge_dict = edge_match_utils.get_idedge_dict(swc_tree)

	for tn in swc_tree_list:
		if tn.is_virtual():
			continue
		if tn.parent.get_id() == -1:
			branch_list.append(branch_list_temp)
			branch_list_temp = []
			branch_list_temp.append(tn.get_id())

			branch_node_list.append(branch_node_list_temp)
			branch_node_list_temp = []
			branch_node_list_temp.append(tn)
	...
	# ===========================
	total_length = 0
	IF_ISOLATED_branch_list = []
	print('共有枝杆： %d' % len(branch_list))
	for branch_id, branch_node in zip(branch_list, branch_node_list):
		print('当前枝干长度：%d' % len(branch_id))
		print(branch_id)
		total_length += len(branch_id)

		leaf_node_temp_list = []  # 保存当前枝干的2个叶子结点（或1个）
		for node_id, node_ in zip(branch_id, branch_node):
			node_current = node_  # swc_tree.node_from_id(node_id)
			if len(node_current.children) == 0:
				leaf_node_temp_list.append(node_current)
		# print(len(leaf_node_temp_list))

		# 对每个叶子结点，计算其到其他枝干的距离（distance 根据枝干的长度确定）
		if len(branch_id) < 4:
			min_distance = 10 / resize_radio
		else:
			min_distance = 25 / resize_radio

		IF_ISOLATED_list = []
		for node_leaf in leaf_node_temp_list:
			# print('-----------------------')
			IF_ISOLATED_temp = False

			node_temp_list = edge_match_utils.get_nearby_edges(rtree=test_tree_rtree, point=node_leaf,
			                                                   id_edge_dict=test_tree_idedge_dict, not_self=True,
			                                                   threshold=min_distance)  #

			# min_distance_temp = np.Inf
			if len(node_temp_list) == 0:  # 找不到附近点的（多为孤立点）
				IF_ISOLATED_temp = True
				continue
			else:

				for i in range(len(node_temp_list)):

					if node_temp_list[i][0][0].get_id() in branch_id:  # 排除自身枝干影响
						pass
					else:  # 找到其他枝干
						IF_ISOLATED_temp = False
						break
			...
			IF_ISOLATED_list.append(IF_ISOLATED_temp)
		if False in IF_ISOLATED_list:  # 综合两个种子点信息
			IF_ISOLATED = False
		else:
			IF_ISOLATED = True
		if len(branch_id) > isolated_branch_length_max:  # 长于10的不为独立枝干
			IF_ISOLATED = False
		print(IF_ISOLATED)
		IF_ISOLATED_branch_list.append(IF_ISOLATED)
	print('共有枝杆： %d, 孤立枝干： %d' % (len(branch_list), np.sum(IF_ISOLATED_branch_list)))
	# pause

	k = 0
	for k in range(len(branch_list)):
		if k % 10 == 0:
			print('当前进度%3f' % (k / len(branch_list)))
		branch_state = IF_ISOLATED_branch_list[k]
		branch_node = branch_node_list[k]
		if branch_state:
			for node_ in branch_node:
				swc_tree.remove_node(node_)
	swc_tree.get_node_list(update=True)

	return swc_tree

def remove_noise(swc_tree):
	branch_list = []
	branch_list_temp = []
...
def gen_circle_gaussian_2d(size=[1,64,64], r=32.0, z_offset=0, x_offset=0, y_offset=0):
    z0 = 0
    x0 = size[1]//2
    y0 = size[2]//2
    x0 += x_offset
...
def find_path_edge_2d(img, seed, end_point_list):
    img = img.astype(np.float32)

    
    score_img = np.ones_like(img) * np.inf
    score_img[seed[1]][seed[2]] = 1

    path_img = np.ones_like(score_img).astype(np.float32)
    path_img[seed[1]][seed[2]] = np.inf

    walked_img = np.ones([img.shape[0]+2, img.shape[1]+2]).astype(np.float32) * np.inf
    walked_img_ = np.ones_like(score_img).astype(np.float32)
    walked_img[1:-1,1:-1] = copy.deepcopy(walked_img_)
    walked_img[seed[1]+1][seed[2]+1] = np.inf



    seq_img = np.zeros_like(score_img).astype(np.float32)
    seq_len_img = np.zeros_like(score_img).astype(np.float32)
    seq_len_img[seed[1]][seed[2]] = 1
    ...
    id = 1
    next_node_pos = seed
    while np.min(walked_img[1:-1, 1:-1]) == 1:
        # z_temp = next_node_pos[0]
        x_temp = next_node_pos[1]
        y_temp = next_node_pos[2]

        # print("=========================================")
        # print(z_temp,x_temp, y_temp)

        structure_path_temp = copy.deepcopy(walked_img[x_temp - 1 + 1:x_temp + 2 + 1,y_temp - 1 + 1:y_temp + 2 + 1]) * structure
        
        # print(structure_path_temp)

        next_node_list_temp = np.where(structure_path_temp == 1)
        # print(len(next_node_list_temp[0]))
        
        for node_temp_id in range(len(next_node_list_temp[0])):
            # z_ = next_node_list_temp[0][node_temp_id] + z_temp - 1
            x_ = next_node_list_temp[0][node_temp_id] + x_temp - 1
            y_ = next_node_list_temp[1][node_temp_id] + y_temp - 1
            
            score_current = score_img[x_][y_]

            
            range_ = np.max(img) - np.min(img)
            k_ = 10
            score_1 = np.exp(k_*((img[x_][y_] - np.min(img)) / range_))-1
            score_2 = np.exp(k_*((img[x_temp][y_temp] - np.min(img)) / range_))-1
            score_new = score_img[x_temp][y_temp] + (score_1 + score_2)/2
            

            score_img[x_][y_] = min(score_current, score_new)
            seq_img[x_][y_] = id
            id = id+1
            seq_len_img[x_][y_] = seq_len_img[x_temp][y_temp] + np.sqrt((x_ - x_temp) ** 2 + (y_ - y_temp) ** 2)


            walked_img[x_ + 1][y_ + 1] = np.inf
        

        #更新当前位置，已经遍历
        walked_img[x_temp+1][y_temp+1] = np.inf

        max_index = np.argmin(score_img*path_img)

        x_new, y_new = np.unravel_index(max_index, score_img.shape)
        path_img[x_new][y_new] = np.inf

        next_node_pos = [0, x_new, y_new]
        # print(next_node_pos)
        ...
        if score_img[x_new][y_new]==255:
            break

        path_img[x_temp][y_temp] = np.inf
...
def get_pos_image_2d(image, node_list, pos, shape):
    x_half = shape[0]//2
    y_half = shape[1]//2
    ...
    mark = np.zeros((1, shape[0],shape[1]))
    mark_shape = (1, shape[0],shape[1])
    
    start_point = [1, x_half, y_half]  # (x, y) 起点坐标
    ...
    if len(node_list)<2:
        ...
        for i in range(2, len(node_list)+1):
            ...
            end_node = Vertex(0, 0, end_point[0]-node_list[-1][0], end_point[1]-node_list[-1][1]+32, end_point[2]-node_list[-1][2]+32, 1, -1)
...
def get_network_predict_vecroad_2d(org_lab_temp, org_skl_temp, image, node_list_walked, image_walk, shape, model_test, device, vector_bins, tracing_strategy_flag, steps=40): # 60 for drive
    image = copy.deepcopy(image) / 255 #* 2 - 1
    image_walk = copy.deepcopy(image_walk) / 255
    
    data_transform = torchvision.transforms.Compose([])
    
    # seq_len, _,_,_ = image.shape
    image_tensor = np.zeros(dtype=np.float32, shape=[1, 4, *shape])
    image_tensor[0,0:3,:,:] = copy.deepcopy(image)
    image_tensor[0,3,:,:] = copy.deepcopy(image_walk[0])
    ...
    for x_batch in test_loader:
        ...
        img_org, img_walk = batch_input[0][0:3], batch_input[0][3]
        # print(img_org.shape, img_walk.shape)
        
        img_org = img_org.reshape(1,3,data_shape[0],data_shape[1])
        img_walk = img_walk.reshape(1,1,data_shape[0],data_shape[1])
        
        # 网络预测
        y_skl_pred, y_point_pred, y_edge_pred = model_test(img_org, img_walk)
        ...
        pred_tb = pred_tb.reshape(data_shape[0],data_shape[1])
        ...
        # road 3
        mask_r = r
        point_temp_mask = gen_circle_gaussian_2d(size = [1,64,64], r=mask_r, z_offset=0, x_offset=0, y_offset=0)*255
        ...
        pred_edge = pred_edge.reshape(data_shape[0],data_shape[1]).astype(np.uint8)
        ...
        if np.sum(pred_tb) == 0:
            max_row = 0
            max_col = 0
            next_node_pos.append([0, max_row+node_list_walked[-1][1], max_col+node_list_walked[-1][2]])
        else:
            point_th = np.max(pred_tb)//2
            pred_tb_temp = copy.deepcopy(pred_tb)
            while np.max(pred_tb_temp)>point_th:
                max_index = np.argmax(pred_tb_temp)
                max_row, max_col = np.unravel_index(max_index, pred_tb_temp.shape)
                next_node_pos_patch.append([0, max_row, max_col])
                
                max_row = max_row - data_shape[0]//2
                max_col = max_col - data_shape[1]//2

                
                point_temp_mask = gen_circle_gaussian_2d(size = [1,data_shape[0],data_shape[1]], r=mask_r, z_offset=0, x_offset=max_row, y_offset=max_col)*255

                th = 128
                point_temp_mask[point_temp_mask<=th]=0
                point_temp_mask[point_temp_mask>th]=255
                point_temp_mask = point_temp_mask/255
        ...
        print('可能的点：', next_node_pos_patch)
        # 保存结果
        img_org = img_org.cpu().detach().numpy()
        img_org = np.transpose(img_org[0], (1, 2, 0))
        img_org = img_org*255 
        img_org = img_org.astype(np.uint8)
        
        img_walk = img_walk.cpu().detach().numpy()
        img_walk = img_walk.reshape(64,64)
        img_walk = img_walk*255
        img_walk = img_walk.astype(np.uint8)
        
        

        path_img, score_img , _ ,_, next_node_pos_ = find_path_edge_2d(pred_edge, seed, next_node_pos_patch)
        ...
        if steps != 1:
            max_depth = 0
            max_row = next_node_pos_[1] - data_shape[0]//2 
            max_col = next_node_pos_[2] - data_shape[1]//2 
            ...
            next_node_pos_ = [0,0,0]
            next_node_pos_[0] = 0
            next_node_pos_[1] = -max_row/4 + node_list_walked[-1][1]
            next_node_pos_[2] = -max_col/4 + node_list_walked[-1][2]
        
            next_node_pos.append(next_node_pos_)
        else:
            for node_temp_ in next_node_pos_patch:
                max_depth = 0
                max_row = node_temp_[1] - data_shape[0]//2 
                max_col = node_temp_[2] - data_shape[1]//2 
        ...
        for node_pos_temp in next_node_pos:
            ...
            node_exist_next = org_lab_temp[node_pos_next[1]][node_pos_next[2]]
...
def get_node_centerline_from_image_2d(node_pos, r_temp, org_image_centerline):
    ...
    radius_region = np.zeros([1, 2 * r_temp_ + 1, 2 * r_temp_ + 1])
...
def tracing_strategy(current_node_dict, tree_new_rtree, tree_new_idedge_dict):
    ...
    if node_id == 1:
        ...
        node_temp_list = edge_match_utils.get_nearby_edges(rtree=tree_new_rtree, point=son_node_temp, id_edge_dict=tree_new_idedge_dict, threshold=node_r*3)#
...
def tracing_strategy_single_vecroad_2d_test(org_image, org_lab_temp, org_skl_temp, node_list_walked, seed_node, seed_node_dict, tree_new, tracing_strategy_flag, device_info, data_info, r_tree_info):
    ...
    while len(node_pool) != 0:
        ...
        if end_tracing == True:
            ...
            seed_node_img, seed_node_img_walked = get_pos_image_2d(org_image, node_list_walked, node_pos, SHAPE)
            # seed_node_img = seed_node_img.reshape(3,*SHAPE)
            seed_node_img = np.transpose(seed_node_img, (2, 0, 1))
            seed_node_img_walked = seed_node_img_walked.reshape(1,*SHAPE)
            
            
            exist, next_node_pos = get_network_predict_vecroad_2d(org_lab_temp, org_skl_temp, seed_node_img, node_list_walked, seed_node_img_walked, SHAPE, model_test, device, vector_bins, tracing_strategy_flag, steps)
            ...
            for node_temp_id in cos_sim_sorted_list_index:
                ...
                node_pos_next = [round(z_temp_new_ave), round(x_temp_new_ave), round(y_temp_new_ave)]
                print('预测点位置：', node_temp_id+1,"|", round(z_temp_new_ave), round(x_temp_new_ave)-64, round(y_temp_new_ave)-64)

                # 判断是否在图像区域外
                if in_range(round(z_temp_new_ave), 0, 1) is False or in_range(round(x_temp_new_ave), data_shape[0]//2, org_image.shape[0]-data_shape[0]//2) is False or in_range(round(y_temp_new_ave), data_shape[1]//2, org_image.shape[1]-data_shape[1]//2) is False:
