...
def getChildRadius(depth, max_r = 3.0, min_r = 1.0, decay_rate = 0.9):
    return np.random.uniform(min_r,max_r) * (decay_rate)**(depth)
...
def simulate3DTreeModel_spiralline(MAX_BOX_WIDTH, theta_circle = 5, R = 20, data_type = np.uint8):
    # MAX_TREE_DEPTH = 50
    ...
    # Create root node
    node_count = 0
    root_r = getChildRadius(1, max_r = 3.0, min_r = 0.5, decay_rate = 1)

    # y,x,z
    gap = 50
    pos_lib = [[MAX_BOX_WIDTH[0] // 2, MAX_BOX_WIDTH[1] // 2, gap]]
    ...
    theta = np.arange(1e-5, theta_circle * 2 * np.pi, 0.5)
    x_pos = MAX_BOX_WIDTH[0]//2 + R * np.sin(theta)
    y_pos = MAX_BOX_WIDTH[1]//2 + R * np.cos(theta)
    z_pos = np.arange(gap, MAX_BOX_WIDTH[2]-gap, (MAX_BOX_WIDTH[2]-gap*2)/len(x_pos))



    for num in range(len(x_pos)):
        root_node = dq[0][0]
        root_depth = dq[0][1]
        # print(root_depth)
        # print(root_node, root_depth)
        # pause
        dq.popleft()
        ...
        if root_depth < len(x_pos):
            ...
            if root_node.idx == 1:  # 根节点单独处理
                ...
                child_pos[2] = int(z_pos[num])

                child_r = getChildRadius(root_depth + 1, max_r = 3.0, min_r = 0.5, decay_rate = 1)
    ...
    if data_type == np.uint8:  # 8bit image
        if external_feature == True:
            forground = np.random.randint(100, 300)
            background_mean = np.random.randint(20, 100)
            background_std = np.random.randint(5, 10)
        else:
            forground = np.random.randint(20, 250)
            background_mean = 1
            background_std = 0
